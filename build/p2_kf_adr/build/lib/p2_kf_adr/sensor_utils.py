...
def generate_noisy_measurement(pose, v, omega, noise_std=None):
    """
    Adds Gaussian noise to the observation [|pos|, theta, v] and returns a 3x1 vector.

    :param pose: Tuple (x, y, theta)
    :param v: Linear velocity (float)
    :param omega: Angular velocity (not used)
    :param noise_std: Optional std deviations for each component (length 3)
    :return: Noisy observation as a (3, 1) numpy array
    """
    if noise_std is None:
        noise_std = np.array([0.02, 0.01, 0.02])  # std for [|pos|, theta, v]

    x, y, theta = pose
    position_magnitude = np.sqrt(x**2 + y**2)

    noise = np.random.normal(0, noise_std)
    z = np.array([[position_magnitude + noise[0]],
                  [theta + noise[1]],
                  [v + noise[2]]])
    return z
